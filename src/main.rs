...
impl Resource {
    async fn new() -> Result<Resource, macroquad::Error> {
    ...
    pub async fn load() -> Result<(), macroquad::Error> {
        let resource_loading = start_coroutine(async move {
            let resources = Resource::new().await.unwrap();
...
async fn main() -> Result<(), macroquad::Error> {
    ...
    // let resources = Resource::new().await?;
    Resource::load().await?;
    let resources = storage::get::<Resource>();
    ...
    loop {
        ...
        match game_state {
            GameState::MainMenu => {
                ...
                let center_pos = vec2((w - window_size.x) / 2.0, (h - window_size.y) / 2.0);

                widgets::Window::new(hash!(), center_pos, window_size)
                    .movable(false)
                    .ui(&mut root_ui(), |ui| {
                        ui.label(vec2(88.0, -50.0), "Main Menu");
                ...
                //    .position(center_pos)
                //    .ui(&mut root_ui(), |ui| {
                //        ui.label(vec2(88.0, -34.0), "Main Menu");
                //        if ui.button(vec2(65.0, 25.0), "Play") {
                //            squares.clear();
                //            bullets.clear();
                //            explosions.clear();
                ...
                //        }
                //    });
                //root_ui().group(hash!(), center_pos, window_size, |ui| {
                //    ui.label(vec2(88.0, -34.0), "Main Menu");
                //    if ui.button(vec2(65.0, 25.0), "Play") {
                //        squares.clear();
                //        bullets.clear();
            ...
            GameState::Playing => {
                ...
                for square in &squares {
                    ...
                    //    GREEN,
                    //);
                    let (enemy_texture, enemy_frame) = match Some(square.size) {
                        Some(x) if x < 32.0 => (&resources.enemy_small_texture, &enemy_small_frame),
                        Some(x) if x >= 32.0 && x < 50.0 => {
                            (&resources.enemy_medium_texture, &enemy_medium_frame)
                        }
                        Some(x) if x >= 50.0 => (&resources.enemy_big_texture, &enemy_big_frame),
                        _ => (&resources.enemy_small_texture, &enemy_small_frame),
                    };
                    draw_texture_ex(
                        enemy_texture,
                        square.x - square.size / 2.0,
                ...
                let highstore_text = format!("Highscore: {}", high_score);
                let text_dimensions = measure_text(highstore_text.as_str(), None, 25, 1.0);
                draw_text(
                    highstore_text,
